...
const EditTourModal = ({ show, onClose, currentTour, setTours }) => {
  const [formData, setFormData] = useState(currentTour || {});
  const [errors, setErrors] = useState({});
  const [activeTab, setActiveTab] = useState('basic');

  useEffect(() => {
    if (currentTour) {
      // Khởi tạo images với isMain (hình đầu tiên là chính nếu chưa có)
      const images = (currentTour.images || []).map((url, index) => ({
        url,
        file: null,
        isMain: index === 0 && !currentTour.images.some((img, i) => i !== index && img.isMain),
      }));
      setFormData({ ...currentTour, images });
    }
  }, [currentTour]);
  ...
  const validateForm = () => {
    ...
    if (!formData.images || formData.images.length === 0) {
      newErrors.images = 'Phải có ít nhất 1 hình ảnh';
    }
  ...
  const handleUpdateTour = (e) => {
    ...
    setTours((prev) => prev.map((tour) => (tour._id === formData._id ? updatedTour : tour)));
    onClose();
  };
  ...
  const onDrop = useCallback((acceptedFiles) => {
    const newImages = acceptedFiles.map((file) => ({
      ...
      file,
      isMain: formData.images.length === 0, // Hình đầu tiên là chính nếu chưa có
    }));
    setFormData((prev) => ({
      ...prev,
  ...
  return (
    <div className="fixed inset-0 bg-black/50 bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white p-6 rounded-lg max-w-4xl max-h-[90vh] overflow-y-auto">
        <div className="mb-4">
          <h3 className="text-xl font-bold text-black">Chỉnh sửa tour</h3>
          {currentTour?.total_slots_booked > 0 && (
            <p className="text-amber-600 font-medium mt-2">Cảnh báo: Cập nhật có thể ảnh hưởng đến các đơn đặt.</p>
          )}
        </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                ...
                <input
                  type="number"
                  min="0"
                  value={formData.representative_price?.adult || 0}
                ...
                <div className="grid grid-cols-2 gap-2">
                  {['Ăn uống', 'Vé tham quan', 'Hướng dẫn viên', 'Vé máy bay', 'Khách sạn', 'Xe đưa đón'].map((service) => (
                    <div key={service} className="flex items-center space-x-2">
                      ...
                      />
                      <label htmlFor={`edit-service-${service}`} className="text-sm text-gray-700">
                        {service}
                      </label>
                    </div>
                  ))}
                ...
                <div key={index} className="bg-gray-50 p-4 rounded-lg">
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center">
                      <label htmlFor={`edit-day-${index}`} className="text-sm font-medium mr-2">
                        Ngày:
                      </label>
                      <input
                        type="number"
