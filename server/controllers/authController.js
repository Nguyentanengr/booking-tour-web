...
exports.login = async (req, res) => {
    ...
    try {
        ...
        const { accessToken, refreshToken } = generateTokens({ _id: user._id, role });
        successResponse(res, { accessToken, refreshToken, user: { _id: user._id, name: user.full_name, email: user.email, role } }, "Đăng nhập thành công.");
    } catch (error) {
...
exports.forgotPassword = async (req, res) => {
    ...
    try {
        ...
        successResponse(res, { message: `Một mã OTP đã được gửi đến email của bạn.` });
...
exports.verifyOtp = async (req, res) => {
    ...
    try {
        ...
        successResponse(res, { message: 'Xác thực OTP thành công.' });
    } catch (error) {
        errorResponse(res, 'Lỗi server: ' + error.message, 500);
    }
};
...
exports.resetPassword = async (req, res) => {
    ...
    try {
        ...
        successResponse(res, { message: 'Mật khẩu đã được cập nhật thành công.' });
    } catch (error) {
        errorResponse(res, 'Lỗi server: ' + error.message, 500);
    }
};
...
exports.refreshToken = async (req, res) => {
    ...
    try {
        ...
        const { accessToken } = generateTokens({ _id: decoded._id, role: decoded.role }, 'access');
